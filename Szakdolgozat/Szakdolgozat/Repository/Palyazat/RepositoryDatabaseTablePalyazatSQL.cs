...
namespace Szakdolgozat.Repository
{
    partial class RepositoryDatabaseTablePalyazatSQL
    {
        ...
        public List<Palyazat> getFilteredPalyazatokFromDatabaseTable(string keresettTipus, string keresettSzoveg)
        {
            List<Palyazat> palyazatok = new List<Palyazat>();
            MySqlConnection connection = new MySqlConnection(connectionString);
            try
            {
                connection.Open();
                string query = Palyazat.getFilteredRecords(keresettTipus, keresettSzoveg);
                MySqlCommand cmd = new MySqlCommand(query, connection);
                MySqlDataReader dr;
                dr = cmd.ExecuteReader();
                string szakmaiVezetoNeve = "";
